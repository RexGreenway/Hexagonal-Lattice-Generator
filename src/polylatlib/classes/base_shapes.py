...
class Shape:
    ...
    def __init__(self):
        ...
        -------
        >>> A = Shape()
        >>> for i in range(5):        >>>     A.add_vertex(i) # adding vertices 0 through 4 to A.
        >>> A.add_edge(0, 1)
        >>> A.add_edge(1, 8, 4)
        >>> print(A.vertices)
        [0, 1, 2, 3, 4, 8]
    ...
    def __str__(self):
        ...
        -------
        info : string
            Basic information of the shape object. Indicates the shape type, number of vertices,
            and number of sides.
    ...
    def add_vertex(self, vertex_for_adding, position=None, size: int = 4, colour="b"):
        """
        Adds a desired vertex to the shape, with associated properties; position, size, and
        colour.
        ...
        position : (x, y) - 2D Cartesian Coordinate, Default = None, optional
            ...
            have no position by default allowing for the creation of a graph-like object.
        size : int, Default = 4, optional
            Size property to be utilised upon drawing of the shape.
        colour : colour, Default = "b", optional
            Colour property to be utilised upon drawing of the shape. Colour can be chosen from
        ...
        be utilised in the creation of edges resulting in a shape with no set structure except
        ...
        Size and colour are utilised in the drawing of the shape with size indicating the radius
        ...
        if vertex_for_adding not in self.vertices:
            ...
            if not check_if_coord(position) and position is not None:
            ...
            info = {"position": position, "size": size, "colour": colour}
    ...
    def update(self, item_for_update, prop, value):
        ...
        proceeds to call the specific update method as necessary.
    ...
    def update_vertex(self, vertex_for_update, prop, value):
        ...
        vertex_for_update : vertex
            A preexisting vertex in the shape to be updated.
    ...
    def get_vertex_info(self, desired_info):
        ...
        vertex_info : dictionary
            Property dictionary with vertices as keys, and the desired property ('desired_info')
            as values.
    ...
    def add_edge(self, vertex_one, vertex_two, weight=1, colour="k"):
        ...
        weight : int > 0, Default = 1, optional
            Weight property to be utilised upon drawing of the shape.
        colour : colour, Default = "k", optional
            Colour property to be utilised upon drawing of the shape. Colour can be chosen
        ...
        -------
        >>> A = Shape()
        >>> A.add_edge(1, 2)                                # Adding edge between to new vertices, 1, and 2.
        >>> A.add_edge("Hello", "World", 3)                 # An edge between 'Hello' and 'World' with weight 3.
        >>> A.add_edge("Hello", "xmpl", colour = "r")       # Establishing edge between existing "Hello" and new "xmpl".
        >>> print(A.edges)
        ...
        (a, b) = (b, a). Edge properties are weight and size. Weight references the 'thickness'
        of the edge line and colour is self-explanatory. These properties utilised in the
        drawing of shapes.

        Vertices not pre-existing within the shape are automatically generated and added with
        ...
        if (vertex_one, vertex_two) not in self:
            ...
            info = {"weight": weight, "colour": colour}
            ...
            raise PolyLatError(
                f"Edge '{(vertex_one, vertex_two)}' already exists in the shape."
            )
    ...
    def update_edge(self, edge_for_update, prop, value):
        ...
        edge_for_update : 2-tuple, vertex pair
            A preexisting edge in the shape to be updated.
        ...
        Notes
        -----
        This method performs the relevant update method depending upon the given edge.
    ...
    def get_edge_info(self, desired_info):
        ...
        Notes
        -----
        This method returns a very usable collection of all edges in the shape with their associated
        desired property.
    ...
    def get_edge_vectors(self):
        ...
        same vector with negative x, and y, values.

        This method calculates and returns the true edge vectors for all edge present within the
        shape. Calculation of these vectors from the property dictionaries is the preferred
        ...
        for edge in self.edges:
            ...
            if pos_one is None or pos_two is None:
                raise TypeError(
                    "Edge vectors cannot be generated as one or more vertices in an edge do not have a position."
                )
    ...
    def generate_shape(self, vertex_pos, shape_name, vectors):
        ...
        shape_name : string, int, or float
            This is the overriding shape name dictating all the vertex names within the shape.
        ...
        Example
        -------
        >>> vertices = [(0, 1), (1, 1), (-1, 2)]
        >>> A = pl.Shape()
        >>> A.generate_shape((0, 0), "Example", vertices)
        >>> A.draw_shape()
        ...
        have multiple vertices occupying the same position. This method does not close the shape
        automatically.
        """
        edge_list = []
        for k in range(len(vectors) + 1):
            ...
            for key in vertex_dict.keys():
                if (vertex_pos[0] - vertex_dict[key][0]) ** 2 + (
                    vertex_pos[1] - vertex_dict[key][1]
                ) ** 2 <= (1 / 100) ** 2:
    ...
    def draw_shape(self, axis="off"):
        ...
        Notes
        -----
        Uses the Matplotlib library to visualise the current shape, accounting for vertex
...
class Lattice(Shape):
    ...
    def get_shape_sides(self):
        ...
        IMPLEMENT DOCUMENTATION
        """
        # This returns the No. of sides of the shape in the lattice....
