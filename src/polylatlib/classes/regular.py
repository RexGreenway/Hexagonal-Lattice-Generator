...
from polylatlib.classes.base_shapes import Lattice, Polygon
...
class RegularPolygon(Polygon):
    ...
    def __init__(self, sides: int, edge_length, centre, rotation):
        ...
        if self.sides < 3:
            raise ValueError(
                f"Argument 'sides' = {self.sides}. A regular polygon have at least 3 sides."
            )
        elif self.edge_length < 0:
            raise ValueError(
                "Argument 'edge_length' = {self.edge_length}. Edges cannot be of negative length.".format(
                    self=self
                )
            )
    ...
    def __generate_polygon_vectors(self):
        ...
        Notes
        -----
        This private method is used in the generation of regular polygon objects.
        """
        edge_vectors = []
...
class EquilateralTriangle(RegularPolygon):
    ...
    rotation : angle, Default = 0, optional
        The angle, in degrees, to rotate the polygon around its centre anti-clockwise.
    ...
    def generate_lattice_circular(self, layers: int):
        ...
        -------
        lattice : Lattice
            Lattice object of equilateral triangles in circular layers centred on the generating
            shape's centre.

        Notes
        -----
        Triangles in circular lattices have alternating orientation for each layer.
        """
        polar_vectors = []
        for i in range(2 * self.sides):
            polar_vectors.append(
                (self.edge_length, 30 + (i + 1) * self.int_angle + self.rotation)
            )
                ...
                if layer % 2 == 0:  # Odd Layers
                    ...
                    for i in range(3):
                        ...
                        for _ in range(int(layer / 2)):
                            shape += 1
                            lattice.generate_shape(vertex_pos, shape, triangle_one)
                            vertex_pos = add_vectors(
                                vertex_pos, chg_vectors[(2 * i) + 1]
                            )
                else:  # Even Layers
    ...
    def generate_lattice_stacked(self, rows, columns):
        ...
        -------
        lattice : Lattice
            Lattice object of equilateral triangles in stacked, row by column, layers centred on
            the generating shape's centre.

        Notes
        -----
        Triangles in stacked lattices have alternating orientation row-wise and column-wise.
        ...
        for i in range(rows):
            ...
            if i % 2 == 0:
                for j in range(columns):
                    if j % 2 == 0:
                        lattice.generate_shape(
                            col_pos, str(i) + "." + str(j), edge_vec[(i % 2)]
                        )
                    else:
                        lattice.generate_shape(
                            col_pos, str(i) + "." + str(j), edge_vec[(i % 2) - 1]
                        )
...
class Square(RegularPolygon):
    ...
    def generate_lattice_circular(self, layers: int):
        ...
        for layer in range(layers):
            radius_vec = (
                round((layer * 2 * self.radius_vec[0]) + self.radius_vec[0], 3),
                round((layer * 2 * self.radius_vec[1]) + self.radius_vec[1], 3),
            )
    ...
    def generate_lattice_stacked(self, rows, columns):
        ...
        -------
        lattice : Lattice
            Lattice object of squares in stacked, row by column, layers centred on
            the generating shape's centre.
...
class Hexagon(RegularPolygon):
    ...
    def generate_lattice_circular(self, layers: int):
        ...
        for i in range(self.sides):
            polar_vectors.append(
                (vector_length, i * (self.theta) + self.theta / 2 + self.rotation)
            )
        chg_vectors = change_to_cart_list(polar_vectors)
    ...
    def generate_lattice_stacked(self, rows, columns):
        ...
        -------
        lattice : Lattice
            Lattice object of hexagons in stacked, row by column, layers centred on
            the generating shape's centre.
